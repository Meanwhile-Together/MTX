...
# MTX command: mtx fixes root-paths [--workspace DIR] [DIR...]
...
source "$MTX_ROOT/fixes/root-paths-lib.sh"

mtx_fixes_root_paths_usage() {
  echo "Usage: mtx fixes root-paths [--workspace DIR] [DIR...]" >&2
...
mtx_fixes_root_paths_one() {
  ...
  [ -n "$p" ] || return 0
  if [ ! -e "$p" ]; then
    echo "mtx fixes root-paths: skip (missing): $p" >&2
    return 0
  fi
  p="$(cd "$p" && pwd)"
  if [ -f "$p/config/server.json" ]; then
    echo "==> mtx fixes root-paths: org $p (server.json paths + payloads/)"
    mtx_predeploy_normalize_source_paths_from_server_json "$p" || return 1
    if [ -d "$p/payloads" ]; then
      local sub
      for sub in "$p/payloads"/*/; do
        [ -d "$sub" ] || continue
        echo "==> mtx fixes root-paths: payloads/$(basename "$sub") under $p"
  ...
  if [ -f "$p/package.json" ]; then
    echo "==> mtx fixes root-paths: app dir $p"
...
mtx_fixes_root_paths_scan_workspace() {
  local ws="$1"
  ws="$(cd "$ws" && pwd)"
  echo "==> mtx fixes root-paths: scanning workspace $ws"
  local cand
...
mtx_fixes_root_paths_main() {
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --workspace)
        workspace="${2:-}"
        [ -n "$workspace" ] || { echo "mtx fixes root-paths: --workspace needs a directory" >&2; return 1; }
        shift 2
        ;;
      *)
        paths+=("$1")
  ...
  echo "==> mtx fixes root-paths: done"
}
