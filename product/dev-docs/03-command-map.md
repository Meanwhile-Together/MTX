...
| Create a new payload app repo | **`mtx create payload [name]`** | Clones from `MTX_PAYLOAD_TEMPLATE_REPO` (default `template-payload`); creates `payload-*` on GitHub; prints `server.apps` snippet. Legacy: `mtx create [name]` without `payload` still works—prefer explicit kind ([MTX_COMMAND_SURFACE.md](../../docs/MTX_COMMAND_SURFACE.md)). |
| Register an existing payload on the current host | **`mtx payload install <payload-id>`** | From project-bridge or `org-*` host root; updates `config/server.json` / `server.json` `apps[]`. See [MTX_COMMAND_SURFACE.md](../../docs/MTX_COMMAND_SURFACE.md). |
