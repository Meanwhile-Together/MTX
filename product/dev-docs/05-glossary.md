...
**Payload template (starter)** — Default repo name **`template-payload`** (single-app SPA); org host default is **`template-org`**. Any **`template-*`** from **`mtx create template`** can be pointed at with **`MTX_PAYLOAD_TEMPLATE_REPO`**. See [MTX_SCAFFOLDING_MODEL.md](https://github.com/Meanwhile-Together/project-bridge/blob/main/docs/MTX_SCAFFOLDING_MODEL.md).
