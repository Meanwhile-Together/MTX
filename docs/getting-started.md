...
3. **Add shebang, `desc`, and `set -e`** in the first 30 lines. Example:
   ...
   - **Optional:** Add **`nobanner=1`** only if you want the 24h banner skipped when this script runs (see **mtx-patterns.md** § nobanner). Script and precond output (echo, echoc) always prints; only **mtx_run** subprocesses are quiet at default.
