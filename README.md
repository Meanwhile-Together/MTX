...
- **New repos from template:** **`mtx create payload`** or **`mtx payload create`** → **`payload-*`** · **`mtx create org`** or **`mtx org create`** → **`org-*`** · **`mtx create template`** or **`mtx template create`** → **`template-*`** (source repo defaults to **`payload-basic`**; set **`MTX_TEMPLATE_SOURCE_REPO`** to clone from another template). **`mtx create`** alone = payload. Requires **`gh`** for create/push (or **`MTX_CREATE_SKIP_GITHUB=1`** for local-only). See [docs/MTX_CREATE_AND_DEPLOYMENT_FLOW.md](docs/MTX_CREATE_AND_DEPLOYMENT_FLOW.md).
