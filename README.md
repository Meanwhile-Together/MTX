...
- **Scaffolding:** **`mtx create payload`** / **`mtx payload create`** → **`payload-*`** app repos (clone from **`MTX_PAYLOAD_TEMPLATE_REPO`**, default **`template-basic`**). **`mtx create template`** / **`mtx template create`** → run **from a payload repo root**; snapshots it into **`template-*`** next to MTX (`template-` = payload templates only) (point **`MTX_TEMPLATE_SOURCE_REPO`** / **`MTX_PAYLOAD_TEMPLATE_REPO`** at them for future `mtx create payload`). **`mtx create org`** / **`mtx org create`** → **`org-*`** when you need a **separate org product repo** (default is **one shared org payload** + config for tenants). **`mtx create`** alone = payload. Requires **`gh`** for create/push (or **`MTX_CREATE_SKIP_GITHUB=1`** for local-only). Story: [docs/MTX_SCAFFOLDING_MODEL.md](docs/MTX_SCAFFOLDING_MODEL.md) · flow: [docs/MTX_CREATE_AND_DEPLOYMENT_FLOW.md](docs/MTX_CREATE_AND_DEPLOYMENT_FLOW.md).
