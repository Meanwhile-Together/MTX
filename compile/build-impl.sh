# MTX compile: server / backend / prepare:railway build steps (sourced from build.sh
# after PROJECT_ROOT, TARGET, mtx_run, spinner, and framework verify have been set up).
...
run_prepare_railway_bundle() {
  ...
  # After payload vendor/build: portable MTX pre-deploy (org hook + root-path HTML/Vite fixes). See includes/mtx-predeploy.sh + tools/fixes/root-paths-lib.sh.
