...
# 2) Payload root-path normalization — see fixes/root-paths-lib.sh (HTML + Vite base; no Node).

_mtx_predeploy_mtx_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
# shellcheck source=../fixes/root-paths-lib.sh
source "$_mtx_predeploy_mtx_root/fixes/root-paths-lib.sh"
